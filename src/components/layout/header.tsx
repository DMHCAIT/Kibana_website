...
export function Header({ announcementText }: { announcementText?: string }) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border bg-background">
      {/* Announcement Banner */}
      {(() => {
        const phrases = announcementText
          ? announcementText.split("·").map((s) => s.trim()).filter(Boolean)
          : ["FREE RETURNS ON ALL ORDERS", "100% PREMIUM VEGAN LEATHER — CRUELTY FREE", "HAND-FINISHED. EVERY PIECE. EVERY TIME.", "FREE SHIPPING ON ORDERS ABOVE ₹999"];
        return (
