...
export function CardPayment({
  ...
  const handleCardPayment = async () => {
    ...
    try {
      ...
      const orderResponse = await fetch("/api/payments/card/process", {
        method: "POST",
      ...
      // Step 2: Open Razorpay Checkout with Card preference
      const options = {
        ...
        handler: async (response: {
          ...
          try {
            ...
            setSuccess(true);
            console.log("✅ Card Payment Successful:", response.razorpay_payment_id);
    ...
    return (
      <div className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
        ...
        <div>
          ...
          <p className="text-xs text-emerald-700">Payment ID: {paymentId}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground text-center">
        🔒 Secured by Razorpay | Visa, Mastercard, RuPay | Real-time deduction
      </p>
    </div>
  );
}
